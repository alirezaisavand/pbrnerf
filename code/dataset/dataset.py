...
from utils import io, geometry, general
...
class EpochSampler:
    ...
    def get(self, idx):
        # TODO: don't use EpochSampler for accessing arrays (no need for shuffling logic)
        sampled_arrays = [arr[idx] for arr in self.reg_arrays]
        return sampled_arrays
...
class NeILFDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.mode = mode
        self.use_brdf_gt = use_brdf_gt
        self.use_depth_map = False
        self.required = required
        self.use_neilfpp_sampler = use_neilfpp_sampler
    ...
    def _load_and_prepare_validation_data(self):
        ...
        self.validation_data = self.val_collate_fn(self.validation_data)
    ...
    def _load_and_prepare_training_data(self):
        ...
        if self.required['position']:
            self.train_mask_pixels = torch.cat(list_mask_pixels, axis=0)                            # [T, HW]
            self.train_mask_pixels = self.train_mask_pixels == 1
        ...
        # mask pixels for more efficient pixel sampling
        if not self.use_neilfpp_sampler and self.required['position']:
            self.train_rgb_pixels = self.train_rgb_pixels[self.train_mask_pixels]
            self.train_rgb_grad_pixels = self.train_rgb_grad_pixels[self.train_mask_pixels]
            self.train_position_pixels = self.train_position_pixels[self.train_mask_pixels]
            if self.required['normal']: self.train_normal_pixels = self.train_normal_pixels[self.train_mask_pixels]
            self.train_uv = self.train_uv[self.train_mask_pixels.reshape(-1)]
            self.train_list_index_pixels = self.train_list_index_pixels[self.train_mask_pixels]
    ...
    def _load_data_from_indexes(self, indexes_to_read):
        ...
        for list_index, image_index in enumerate(tqdm(indexes_to_read)):
            ...
            # if (image_index == '0' or image_index == '94'):
            #     mask = io.load_mask_image_with_prefix(rgb_image_prefix)
            #     rgb_image_ldr = general.hdr2ldr(rgb_image)
            #     rgba_image = np.concatenate([rgb_image_ldr, mask[..., np.newaxis]], axis=-1)
            #     rgba_image = rgba_image * 255
            #     rgba_image = rgba_image.astype(np.uint8)
            #     io.save_image(f"{image_index}.png", rgba_image)
            ...
            if self.required['position']: mask_pixels = mask_map.reshape(1, -1)                 # [1, HW]
            ...
            if self.required['position']: list_mask_pixels.append(mask_pixels)                  # [N][H, W]
    ...
    def __len__(self):
        return self.train_rgb_pixels.shape[0]

    def __getitem__(self, idx):
        if self.use_neilfpp_sampler:
            # uses random indices returns an entire batch at once
            [
                rgb_batch,                                                                          # [B, 3]
                rgb_grad_batch,                                                                     # [B]
                position_batch,                                                                     # [B, 3]
            ...
            # use specific idx to match importance sampling weights
            [
                rgb_batch,                                                                          # [3]
                rgb_grad_batch,                                                                     # [1]
                position_batch,                                                                     # [3]
    ...
    def val_collate_fn(self, batch_list):
        ...
        for entry in batch_list:
            if type(entry[0]) is dict:
                ...
                for k in entry[0].keys():
                    # entry[0]['intrinsics'] [1, 4, 4]
                    ret[k] = torch.stack([obj[k] for obj in entry])
                    # ret['intrinsics'] [9,1,4,4]
    ...
    def collate_fn(self, batch_list):
        # get list of dictionaries and returns input
        # list of lists
        # list of lists, each list with 2 dicts: sample, ground_truth
        # [n_val, 2] -> [2, n_val]
        # val: n_val lists with 2 dicts: sample, ground_truth
        # train: batch lists with 2 dicts: sample, ground_truth
        # dict 1 sample: intrinsics, pose, uv, positions, normals
        # dict 2 sample: rgb, base, metal, rough
        # each dict contains NeILFDataset.__getitem__() outputs, e.g. [B,3]
        batch_list = zip(*batch_list)
        # 2 lists, each list with n_train or n_val dicts: sample, ground_truth
        # _load_and_prepare_validation_data
        ...
        for entry in batch_list:
            if type(entry[0]) is dict:
                ...
                for k in entry[0].keys():
                    ret[k] = torch.stack([obj[k] for obj in entry])
                    # neilfpp sampler
                    # ret['intrinsics'] [1,4096,4,4]
                    # ret['positions'] [1,4096,3]
                    # regular torch loader
                    # intrinsics,pose,uv [4096,4,4]
                    if not self.use_neilfpp_sampler:
                        # add extra 1 dim to match neilfpp
                        # neilfpp loads B elements at once and then the above stack adds 1 dim
                        # regular torch loader 1 element and stacks them, so it is missing the extra added dim
                        ret[k] = ret[k].unsqueeze(0)
    ...
    def extract_pixel_dataset(self):
        H, W = self.image_resolution
        T = self.num_training_images

        masks = self.train_mask_pixels.reshape([T, H, W]) # [T, H, W]

        # Unmask uv and rgb
        uv = torch.zeros([T*H*W, 2]) # [THW, 2]
        uv[self.train_mask_pixels.reshape(-1)] = self.train_uv # [THW, 2]
        uv = uv.reshape([-1, H*W, 2])  # [T, HW, 2]
        # uv = self.train_uv.reshape([-1, H*W, 2])  # [T, HW, 2]
        uv = uv + 0.5

        rgb = torch.zeros([T, H*W, 3]) # [T, HW, 3]
        # rgb = self.train_rgb_pixels.reshape([-1, H, W, 3]) # [T, H, W, 3]
        rgb[self.train_mask_pixels] = self.train_rgb_pixels
        rgb = rgb.reshape([-1, H, W, 3]) # [T, H, W, 3]

        print('NeILFDataset: extracting all pixels')
        list_ray_dirs_norm = []
        list_cam_loc = []
        for list_idx, train_idx in enumerate(tqdm(self.training_indexes)):
            uv_i = uv[list_idx].cuda()
            pose_i = self.all_poses[list_idx].unsqueeze(0).cuda()
            intrinsics_i = torch.from_numpy(self.intrinsics[train_idx]).unsqueeze(0).float().cuda()
            ray_dirs_norm, cam_loc = geometry.get_camera_params(uv_i, pose_i, intrinsics_i)  # [N, 3], [N, 3]
            list_cam_loc.append(cam_loc)
            list_ray_dirs_norm.append(ray_dirs_norm)

        cam_loc = torch.stack(list_cam_loc)
        ray_dirs_norm = torch.stack(list_ray_dirs_norm)
        ray_dirs_norm = ray_dirs_norm.reshape(-1, H, W, 3)

        num_pixels = torch.tensor(rgb.shape[0] * H * W).cuda()
        # Assume all cameras have the same FOV
        f = self.intrinsics['0'][0][0]
        pixel_fov_h = torch.tensor((2/W) * np.arctan(W / (2*f))).cuda()
        pixel_fov_v = torch.tensor((2/H) * np.arctan(H / (2*f))).cuda()

        # detach from computational graph
        cam_loc = cam_loc.detach()
        ray_dirs_norm = ray_dirs_norm.detach()
        rgb = rgb.detach()

        # TODO: convert to PixelDataset class?
        # Maintain these shapes so that it is easier to
        # 1. downsampling
        # 2. broadcasting
        # 3. save on mem storage
        return {
            'num_pixels': num_pixels,
            'pixel_fov_h': pixel_fov_h,
            'pixel_fov_v': pixel_fov_v,
            'origin': cam_loc, # [T, 1, 3]
            'dir': ray_dirs_norm, # [T, H, W, 3]
            'color': rgb, # [T, H, W, 3]
            'masks': masks # [T, H, W]
        }
