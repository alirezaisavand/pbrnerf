...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        ...
        pre_activation_output = x
        x = self.last_active_fun(x)

        return x, pre_activation_output
...
class Geometry(nn.Module):
    ...
    def forward(self, input):
        ...
        ray_dirs, cam_loc = geometry.get_camera_params(uv, pose, intrinsics)        
        ...
        # sdf has no output activation, so we take the final layer linear output as the "pre-activation output"
        pre_activation_output_sdf = feature_vectors
        ...
        rgb_flat, pre_activation_output_rgb = self.rendering_network(points_flat, used_normals, dirs_flat, feature_vectors)
        ...
        if self.bkgd_type == 'uniform':
            ...
            pre_activation_output_bg_sdf = bg_feature_vectors
            # TODO: support loss gradient norm sampling, currently use bkgd_type=None
            bg_rgb_flat, pre_activation_output_bg_rgb = self.bg_rendering_network(None, None, -bg_dirs_flat, bg_feature_vectors)
        ...
        pre_activation_output_geo = {}
        pre_activation_output_geo['sdf'] = pre_activation_output_sdf
        pre_activation_output_geo['rgb'] = pre_activation_output_rgb

        return point_map, normal_map, -ray_dirs, rgb_values, render_masks, num_pixels, aux_output, pre_activation_output_geo
    ...
    def radiance_variance(self, points, normals):
        ...
        rgb_flat, _ = self.rendering_network(points_flat, used_normals, dirs_flat, feature_vectors)
