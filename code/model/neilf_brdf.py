...
class NeILFPBR(nn.Module):
    ...
    def sample_brdfs(self, points, is_gradient=False):
        ...
        x, pre_activation_output_brdf = self.brdf_nn(points)                                                            # [N, 5]
        ...
        return base_color, roughness, metallic, gradients, pre_activation_output_brdf
    ...
    def sample_incident_lights(self, points, incident_dirs):
        ...
        incident_lights, pre_activation_output_Li = self.neilf_nn(nn_inputs)                                          # [N * S, 3]
        # use mono light
        if incident_lights.shape[1] == 1:
            incident_lights = incident_lights.repeat([1, 3])
        return incident_lights.reshape([N, S, 3]), pre_activation_output_Li                                           # [N, S, 3]
    ...
    def plot_point_env(self, point, width):
        ...
        env_map, _ = self.neilf_nn(nn_inputs)
        env_map = env_map.view(-1, width, 3)                               # [N, 3]
    ...
    def rendering_equation(self,
        ...
        f_s, NDF = _f_specular(h_d_n, h_d_o, n_d_i, n_d_o, base_color, roughness, metallic)      # [N, S, 3]

        rgb = ((f_d + f_s) * incident_lights * incident_areas * n_d_i).mean(dim=1)          # [N, 3]

        return rgb, f_d + f_s, f_d, f_s, n_d_i, h_d_n, NDF

    def forward(self, points, normals, view_dirs, geometry):

        # get brdf
        base_color, roughness, metallic, gradients, pre_activation_output_brdf = self.sample_brdfs(
        ...
        rgb, brdf, diffuse_brdf, specular_brdf, n_d_i, h_d_n, NDF = self.rendering_equation(
            view_dirs, normals, base_color, roughness, metallic,
            incident_lights, incident_dirs, incident_areas)                                 # [N, 3], [N, S, 3], [N, S, 1]

        if self.training:
            trace_mask, miss_mask, trace_render_rgb, misc = geometry.trace_and_render(points.detach(), normals.detach(), incident_dirs, sample=1)
            trace_nn_rgb = incident_lights[trace_mask] # [M'L', 3]
            miss_lights = None
            global_lights = None
            trace_inv_sq_dist = misc['trace_inv_sq_dist'] # [M'L', 1]
        else:
            trace_mask = None
            trace_nn_rgb = None
            ...
            miss_lights = None
            global_lights = None
            trace_inv_sq_dist = None

        return rgb, base_color, roughness, metallic, gradients, trace_nn_rgb, trace_render_rgb, miss_lights, global_lights, \
                brdf, diffuse_brdf, specular_brdf, n_d_i, h_d_n, NDF, trace_inv_sq_dist, trace_mask, incident_lights, pre_activation_output_Li, pre_activation_output_brdf
...
class NeILFModel(nn.Module):
    ...
    def forward(self, input):
        ...
        # parse input
        points, normals, view_dirs, geo_rgb, render_masks, total_samples, geo_aux_output, pre_activation_output_geo \
            = self.geometry(input)
        ...
        if self.phase in ['mat', 'joint']:
            ...
            (
                ...
                trace_mask,
                incident_lights,
                pre_activation_output_Li,
                pre_activation_output_brdf
            ) = self.neilf_pbr(
                points[render_masks], normals[render_masks], view_dirs[render_masks],
                self.geometry)
            # Only PBR render points that are within the masks

        if self.phase == 'joint':
            pre_activation_outputs = {
                    'geo': pre_activation_output_geo,
                    'Li': pre_activation_output_Li,
                    'brdf': pre_activation_output_brdf
            }
        ...
        if self.training:
            ...
            if self.phase in ['mat', 'joint']:
                ...
                output['brdf'] = brdf
                output['diffuse_brdf'] = diffuse_brdf
                output['specular_brdf'] = specular_brdf
                output['n_d_i'] = n_d_i
                output['h_d_n'] = h_d_n
                output['NDF'] = NDF
                output['trace_inv_sq_dist'] = trace_inv_sq_dist
                output['trace_mask'] = trace_mask
                output['incident_lights'] = incident_lights
            if self.to_ldr:
                output['gamma'] = self.gamma
            if self.phase == 'joint':
                output['pre_activation_outputs'] = pre_activation_outputs
