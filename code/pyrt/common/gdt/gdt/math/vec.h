...
namespace gdt {
  ...
  // ------------------------------------------------------------------
  template<typename T>
  struct GDT_INTERFACE vec_t<T,6> {
    enum { dims = 6 };
    typedef T scalar_t;
    
    inline __both__ vec_t() {}
    inline __both__ vec_t(const T &t) : x(t), y(t), z(t), u(t), v(t), w(t) {}
    // inline __both__ vec_t(const T &_x, const T &_y, const T &_z) : x(_x), y(_y), z(_z) {}
// #ifdef __CUDACC__
//     inline __both__ vec_t(const int3 &v) : x(v.x), y(v.y), z(v.z) {}
//     inline __both__ vec_t(const uint3 &v) : x(v.x), y(v.y), z(v.z) {}
...
// #endif
    ...
    /*! assignment operator */
    inline __both__ vec_t<T,6> &operator=(const vec_t<T,6> &other) {
      this->x = other.x;
      this->y = other.y;
      this->z = other.z;
      this->u = other.u;
