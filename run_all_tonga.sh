...
# (keep "sphere" since you used it; add common NeRF-Synthetic ones)
DEFAULT_SCENES=(sphere chair drums ficus hotdog lego materials mic ship)

# Optional: set via env when calling, e.g.
# TAGS=exp1 CONS_LOSS=0.1 SPEC_LOSS=0.2 ./run_many_nerfsynthetic.sh chair lego
TAGS="${TAGS:-debug}"
CONS_LOSS="${CONS_LOSS:-0.0}"
SPEC_LOSS="${SPEC_LOSS:-0.0}"
DATA_ROOT="${DATA_ROOT:-/workspace/datasets/nerf_synthetic}"
CONFIG_PATH="${CONFIG_PATH:-configs/config_nerfsynthetic_pbrnerf.json}"
OUT_DIR="${OUT_DIR:-outputs}"
LOG_DIR="${LOG_DIR:-logs_nerfsynthetic}"
...
for SCENE in "${SCENES[@]}"; do
  ...
  run_name="pbrnerf_nerfsynthetic_${SCENE}"
